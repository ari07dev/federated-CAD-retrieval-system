...
# POPPLER path (Configuration or Relative)
POPPLER = os.environ.get("POPPLER_PATH", os.path.join(BASE, "poppler", "Library", "bin"))
if not os.path.exists(POPPLER):
    # Fallback/Warning (Users might have it in PATH)
    POPPLER = None

TOP_K = 3

# Weights for hybrid sketch scoring
CLIP_RAW_WEIGHT = 0.3    # Raw CLIP similarity (image vs image)
CLIP_EDGE_WEIGHT = 0.5   # Edge-domain CLIP similarity (most important)
SHAPE_WEIGHT = 0.2       # Contour shape feature similarity
...
try:
    ...
    if not rows:
        print("NODE_A database empty or query failed")
        rows = []
...
def search_sketch():
    ...
    for idx in candidates:
        ...
        # Cosine similarity of shape features
...
def add_asset():
    try:
      with index_lock:
        ...
        save_path = os.path.join(BASE, "cad_files_a", filename)
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        file.save(save_path)

        # Rel path for DB
        rel_path = f"cad_files_a/{filename}"
